   ...
   <div>
            ...
            <div class="photo_area">
                        ...
                        <div v-for="item in photos" class="photo_wrap a_third_m20_height" >
                            <img :src="item" class="photo a_third_m22_height"/>
                        </div>
                        
                        <div class="photo_wrap a_third_m20_height" style="" @click="addPic" v-show="photos.length<6">
            ...
            vm.receiveData.postData(vm,'/repair/comment',add,'res',function(){
                if(vm.res.success) {
                    // vm.$router.push({path:'/ordersDetail',query:{orderId:vm.$route.query.ordersID}})
                       window.location.href=vm.config.orderpay_commentxiu.url1+"?#/ordersDetail?orderId="+vm.$route.query.ordersID 
                }else {
                        alert("评论失败，请稍后重试！");
                }
                  
            },function() {alert("评论失败，请稍后重试！"); });
        }
...
.addicon {
    background: url(../assets/images/add_img.png) no-repeat;
