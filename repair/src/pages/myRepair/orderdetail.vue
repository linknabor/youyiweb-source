                    ...
                    <div class="lite-divider top-info " style="min-height: 60px;width: 100%">
                        ...
                        <div class="fr">
                            <a :href="oTel" class="weia">
                                <img src="../../assets/images/btn_phone_03.png" class="phone_icon">
                            </a>
                        </div>
                    </div>
   ...
   methods: {
       ...
       showCommentPhoto(i) {
        ...
        cancel() {
            ...
            vm.$router.push({path:'/cancel',query:{orderId:vm.$route.query.orderId}})
        },
        // 完成并支付
        finish() {
               MessageBox.confirm('确认完工后，进入付费操作').then(action => {
                   if(action== 'confirm') {
                        vm.receiveData.getData(vm,"repair/finish/"+vm.item.id,'res',function(){
                           if(vm.res.success) { 
                              location.href=vm.config.repair_orderdetail.url1+"?#/repairPay?orderId="+vm.item.id;
                           }else {
                             alert("系统异常，请稍后重试！");
                           }  
                        },function() {
        ...
        comment() {
            ...
            // vm.$router.push({path:'/commentxiu',query:{orderId:vm.item.id}})
             location.href=vm.config.repair_orderdetail.url1+"?#/commentxiu?ordersID="+vm.item.id
        },
        //返回
        back() {
             vm.$router.push({path:'/myrepair'})
...
/*.time-icon {
    background-image: url(../../assets/images/repair/icon_time_gray.png);
}*/
.icon {
    display: inline-block;
    /* width: 25px;
...
/* 评价 */
.star {
    background: url(../../assets/images/icon_xingxing_weixuanzhong_03.png) no-repeat;
    background-size: 100%;
    width: 20px;
    height: 20px;
    float: left;
 ...
 .star.checked{
  background: url(../../assets/images/icon_xingxing_xuanzhong_03.png) no-repeat;
  background-size: 100%;
}
