<template>
   <div class="addr">
        ...
        <div v-show="currentPage=='list'" style="text-align:center">
            <div class="btn-plain" style="margin-top:30px;color:#3b3937;"  @click="toAddAddress">新建收货地址</div><br/>
            <div class="btn-plain" style="margin-top:10px;color:#3b3937;" v-if="addresses.length>1" @click="setDefaultAddress">设为默认地址</div><br/>
   ...
   methods: {
       ...
       Addresses() {
            vm.receiveData.getData(vm,"addresses",'res',function(){
                if(vm.res.success) {
                        ...
                        let qa = vm.addresses;
                        qa.forEach(function(item,index){
                            console.log(index);
                            console.log(item)
                            if (item.main == true) {
                                console.log(111)
    ...
    //    初始添加地址选择区域
       ...
       check(address) {
        ...
        //设置默认
        setDefaultAddress() {
            if(!vm.checkedAddress == 'undefined') {
                alert('请选择一个地址')
                return;
            }
