<template>
   <div>
        <div v-if="threads_count==0">
		        <img src="../assets/images/bg_publish.jpg"/>
	    </div>
        ...
        <div class="thread-item lite-divider pb15" style="padding-left: 0" v-for="thread in threads" >
            <div class="thread_head">
                <div class="category_img" ><img class="fl" :src="thread.userHead" /></div>
                <div class="thread_category">{{thread.categoryCnName}}</div>
                <div  class="img_delete" @click="deleteThread(thread.threadId)">
                    <img style="height: 25px; width: 25px;  float: right" src="../assets/images/btn_delete.png"/>
                </div>
            </div>
            <div class="p15">
                <div class="ov pb15 fs13" style="color: #3b3937" :class="{unread:thread.hasUnreadComment=='true'}" @click="gotoDetail(thread.threadId)">{{thread.threadContent}}</div>
                <div class="preview_img_layer" >
                    <div v-for="(previewurl,i) in thread.previewLink">
                        <!-- 如果有图片 -->
                        <div class="ov pb15 fl" @click="viewSrcImg(thread.threadId, $index);" >
                ...
                <div class="fs13" style="color: #a6937c; line-height: 23px; width: 100%;">
                    ...
                    <div class="fr pr15" style="text-align: right;" @click="gotoDetail(thread.threadId)"><img style="width: 13px; height: 13px;" src="../assets/images/icon_comment_gray.png">&nbsp;{{thread.commentsCount}}</div>
                </div>
		    </div>
